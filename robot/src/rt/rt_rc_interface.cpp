...
void sbus_packet_complete() {
  ...
  case SWITCH_DOWN:                      // run
    ...
    if (mode_selection_switch == SWITCH_MIDDLE) {
      ...
      if (mode_go_switch == SWITCH_DOWN &&
          initial_mode_go_switch != SWITCH_DOWN) {
    ...
    LOG(INFO) << "selected_mode = " << selected_mode;
  ...
  if (trigger || selected_mode == RC_mode::OFF ||
      selected_mode == RC_mode::RECOVERY_STAND) {
    if (trigger) {
...
float deadband(float command, float deadbandRegion, float minVal,
               float maxVal) {
